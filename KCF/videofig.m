...
%   这不仅可以用于播放和分析标准视频，还可以将任何定制的Matlab图和图形放在顶部。
%   The keyboard shortcuts are:键盘快捷键为：
...
%   还指定播放功能的速度（每秒帧数）和页面向上/向下翻页的帧步长（或默认为空）。
...
%   分别返回图形的手柄，绘图轴和其他手柄（滚动条的图形）。
%   可以调用滚动（F）滚动到框架F，或者没有参数来重新绘制图形。
...
%   Jo鉶 Filipe Henriques, 2010
	...
	%timer to play video定时器播放视频
	play_timer = timer('TimerFcn',@play_timer_callback, 'ExecutionMode','fixedRate');
	
	%main drawing axes for video display主要绘图轴用于视频显示
	...
	function key_press(src, event)  %#ok, unused arguments
		switch event.Key,  %process shortcut keys处理快捷键
		case 'leftarrow',
			scroll(f - 1);
		case 'rightarrow',
			scroll(f + 1);
		case 'pageup',
			if f - big_scroll < 1,  %scrolling before frame 1, stop at frame 1在第1帧前滚动，在第1帧停止
				scroll(1);
			else
				scroll(f - big_scroll);
			end
	...
	function button_down(src, event)  %#ok, unused arguments
		...
		if click_pos(2) <= 0.03,  %only trigger if the scrollbar was clicked仅当滚动条被点击时触发
			click = 1;
			on_click([],[]);
		end
	end
	...
	function on_click(src, event)  %#ok, unused arguments
		...
		%get x-coordinate of click得到x坐标点击
		set(fig_handle, 'Units', 'normalized');
		click_point = get(fig_handle, 'CurrentPoint');
		set(fig_handle, 'Units', 'pixels');
		x = click_point(1);
		
		%get corresponding frame number获取相应的帧号
		new_f = floor(1 + x * num_frames);
		
		if new_f < 1 || new_f > num_frames, return; end  %outside valid range超出有效范围
	...
	function play_timer_callback(src, event)  %#ok
		...
		elseif strcmp(get(play_timer,'Running'), 'on'),
			stop(play_timer);  %stop the timer if the end is reached如果达到结束，请停止计时器
		end
	end

	function scroll(new_f)
		if nargin == 1,  %scroll to another position (new_f)滚动到另一个位置（new_f）
        ...
        %曾经是“drawow”，但是当快速调用并且CPU忙时，它没有正确地让Matlab进程事件（即接近图）。
		pause(0.001)
	end
	
	%convenience functions for argument checks数检查的便利功能
