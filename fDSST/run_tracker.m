...
base_path = 'F:\resarch_dataset\Benchmark\';

%parameters according to the paper
params.padding = 2.0;                   % extra area surrounding the target %padding的倍数
params.output_sigma_factor = 1/16;		% standard deviation for the desired translation filter output% 位置高斯带宽之类的
params.scale_sigma_factor = 1/16;       % standard deviation for the desired scale filter output %尺度高斯带宽之类呢
...
params.scale_step = 1.02;               % Scale increment factor (denoted "a" in the paper) %尺度变化的底a n次方为33
...
params.init_pos = floor(pos([2 1])) + floor(target_sz([2 1])/2); %初始框pos
params.wsize = floor(target_sz([2 1])); %初始框的 h,w
