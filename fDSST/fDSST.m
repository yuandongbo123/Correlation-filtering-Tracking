...
nScalesInterp = params.number_of_interp_scales; %(scale_step）^number_of_interp_scales -= a^n
...
featureRatio = 4;
%% 位置滤波的循环位移标签
output_sigma = sqrt(prod(floor(base_target_sz/featureRatio))) * output_sigma_factor; %高斯标签的带宽
...
if nScales > 0
    ...
    scaleSizeFactors = scale_step .^ scale_exp;  %fdsst的多个尺度的系数17
    interpScaleFactors = scale_step .^ interp_scale_exp_shift;  %插值的多个尺度为33
    
    ys = exp(-0.5 * (scale_exp_shift.^2) /scale_sigma^2);
    ysf = single(fft(ys));
    scale_window = single(hann(size(ysf,2)))'; %scale标签是一维的所以他的加窗也是一维的
    ...
    img_path = [params.video_path 'img/'];
    im = imread([img_path s_frames{1}]);
    
    %force reasonable scale changes
    min_scale_factor = scale_step ^ ceil(log(max(5 ./ sz)) / log(scale_step)); %ceil - 朝正无穷大四舍五入 相当于向上取整
   %最小尺度因子
...
for frame = 1:num_frames,
    %load image
    img_paths = [params.video_path 'img/'];
    im = imread([img_path s_frames{frame}]);
    
    tic();
    
    %do not estimate translation and scaling on the first frame, since we  不估计平移和缩放在第一帧，因为我们  
    %just want to initialize the tracker there  只是想初始化那里的跟踪器 
    if frame > 1
    ...
    if frame == 1
        ...
        num_compressed_dim = min(num_compressed_dim, size(xl_pca, 2)); %比较32维和18维取最小
    else
    ...
    [pca_basis, ~, ~] = svd(data_matrix' * data_matrix); %svd - 奇异值分解
    projection_matrix = pca_basis(:, 1:num_compressed_dim);
    
    hf_proj = fft2(feature_projection(h_num_npca, h_num_pca, projection_matrix, cos_window)); %get_feature 特征转换到傅里叶频域中
    hf_num = bsxfun(@times, yf, conj(hf_proj)); %bsxfun - 对两个数组应用按元素运算（启用隐式扩展）, times - 乘法
    %C = bsxfun(fun,A,B)
    ...
    if nScales > 0
        ...
        if max_scale_dim
            [scale_basis, ~] = qr(bigY, 0); %qr - qr 分解
            %此 MATLAB 函数 返回 qr 分解 A = Q*R 的上三角 R 因子。如果 A 为满矩阵，则 R = triu(X)。如果 A 为稀疏矩阵，则 R = X
