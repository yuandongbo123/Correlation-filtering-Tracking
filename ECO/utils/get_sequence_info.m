function [seq, init_image] = get_sequence_info(seq,video_path)

if ~isfield(seq, 'format') || isempty(seq.format) %isfield - 确定输入是否为结构体(struct)数组字段 返回 1，0
    if isempty(seq)
        seq.format = 'vot';
    else
        seq.format = 'otb';
...
if strcmpi(seq.format, 'otb')
    ...
    seq = rmfield(seq, 's_frames'); %rmfield - 删除结构体中的字段 s = rmfield(s,field)
    ...
    init_image = imread([video_path seq.image_files{1}]);
